...
/**
 * A JPanel that provides a user interface for the active part of the
 * DicomEditor program, including starting the anonymization process and logging
 * the results.
 */
public class RightPanel extends JPanel implements FileListener, ActionListener {
	...
	/**
	 * Class constructor; creates an instance of the RightPanel and initializes the
	 * user interface for it.
	 * 
	 * @param sourcePanel the panel that contains file or directory selected for
	 *                    anonymization.
	 */
...
//		footerPanel.fixVRs.addActionListener(this);
		footerPanel.printIDMapping.addActionListener(this);
		footerPanel.clearPreamble.addActionListener(this);
		footerPanel.setPatientIDs.addActionListener(this);
	}

	/**
	 * The FileListener implementation. This method captures the current file
	 * selection when it changes in the sourcePanel.
	 * 
	...
	/**
	 * The ActionListener for the footer's action buttons. This method starts the
	 * anonymization/VR correction process.
	 * 
	...
	public void actionPerformed(ActionEvent event)
	{
		...
		if (source.equals(footerPanel.printIDMapping)) {
			printIDMapping();
...
//			else if (source.equals(footerPanel.fixVRs)) fixVRs(currentSelection);
			else if (source.equals(footerPanel.clearPreamble))
				clearPreamble(currentSelection);
			else if (source.equals(footerPanel.setPatientIDs))
				setPatientIDs(currentSelection);
	...
	private void printIDMapping()
	{
		File dataMappingFile = new File(this.mappingFile);
		if (this.integerTable != null) {
			java.util.List<String[]> dataLines;
			try {
				dataLines = getMappingLines();
			} catch (IOException e1) {
				e1.printStackTrace();
				return;
			}

			try (PrintWriter pw = new PrintWriter(dataMappingFile)) {
	...
	private java.util.List<String[]> getMappingLines() throws IOException
	{
		java.util.List<String[]> dataLines = new ArrayList<>();

		FastIterator iter;
		iter = integerTable.index.keys();
		...
		if (file.isFile()) {
			...
			if (changeNames) {
				...
				if (!name.matches("[\\d\\.]+")) {
					...
					if (k == -1)
						k = name.length();
					if (name.substring(0, k).endsWith("-no-phi"))
						return;
				}
			...
			if (file.getName().toLowerCase().endsWith(".xml")) {
				...
				result = XMLAnonymizer.anonymize(file, copy, xmlScript, lookupTable.getProperties()).isOK() ? ""
						: "failed";
			} else {
				DAScript dicomScript = DAScript.getInstance(new File(dicomScriptFile));
				LookupTable lookupTable = LookupTable.getInstance(new File(lookupTableFile));
				result = DICOMAnonymizer.anonymize(file, copy, dicomScript.toProperties(), lookupTable.getProperties(),
						this.integerTable, forceIVRLE, renameToSOPIUID).isOK() ? "" : "failed";
				;
			}
			// Report the results
			if (result.equals("")) {
	...
	private void setPatientIDs(File file)
	{
		if (!currentSelection.isDirectory()) {
			resultsPane.appendItem(
					"<br><font color=red><b>" + currentSelection + " is not a directory.</b></font><br></li>");
			return;
		}
		if (file.isFile()) {
			...
			try {
				...
				if (path.length < currentPath.length + 2) {
					resultsPane.appendItem(
							"<br><font color=red><b>Unable to process files in the base directory.</b></font><br></li>");
				} else {
	...
	private void clearPreamble(File file)
	{
		if (file.isFile()) {
			...
			if (result.equals(""))
				...
				resultsPane.appendItem("<br><font color=red><b>Exceptions:</b></font><br>" + result + "</li>");
			return;
		} else {
			File[] files = file.listFiles(filter);
			for (File f : files) {
	...
	private String clearDicomPreamble(File file)
	{
		try {
			...
			raf.read(type);
			if ((type[0] != 0x44) || (type[1] != 0x49) || (type[2] != 0x43) || (type[3] != 0x4D))
...
//			fixVRs = new JButton("Fix VRs");
			...
			printIDMapping = new JButton("Print Mapping");
